...
var resolveFileRef = function(bucketId, fileRef, callback) {
  ...
  client.listFilesInBucket(bucketId, function(err, files) {
    ...
    if (!files || !files.length || Object.prototype.toString.call(files) === '[object String]') {
    ...
    if (!foundFile) {
      return callback('File not found', fileId);
    }
...
var bucketExists = function(client, bucketName, callback) {
  resolveBucketRef.call(client, bucketName, function(err, bucketId) {
    if (err) {
      callback(false);
    }
...
var fileExists = function(client, bucketId, fileName, callback) {
  resolveFileRef.call(client, bucketId, fileName, function(err, fileId) {
    if (err) {
...
var getDecrypter = function(fileId, password, callback) {
  getKeyRing(password, function(keyring) {
    ...
    try {
      decrypter = new storj.DecryptStream(secret);
    }
    catch (err) {
      console.log('Error getting decrypter: %s', err);
      ...
      getFilenameIndex(client, bucketId, function(filenameIndex) {
        ...
        getDecrypter(fileId, password, function(decrypter) {
          ...
          client.createFileStream(bucketId, fileId, function(err, stream) {
            ...
            return callback(null, decrypter);
          });
        });
      });
    });
